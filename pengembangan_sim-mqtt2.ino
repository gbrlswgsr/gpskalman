...
// -------------------------------------------------------
// Fungsi Membuat JSON Data Periodik (VERSI SEDERHANA UNTUK DEVELOPMENT)
// -------------------------------------------------------
String createPeriodicJson() { // Parameter dihilangkan
    StaticJsonDocument <1024> jsonDoc; // Ukuran buffer disesuaikan

    jsonDoc["machine_id"] = "1"; // ID Mesin statis
    jsonDoc["latitude"] = lat_deg;
    jsonDoc["longitude"] = lon_deg;
...
void setup() {
    ...
    if (!myGNSS.begin(Serial1)) { Serial.println("GPS tidak terdeteksi."); }
        ...
        bool freqSet = false;
        if (myGNSS.setNavigationFrequency(5)) { Serial.println("GPS OK, Nav Freq 5 Hz"); freqSet = true; } 
        else if (myGNSS.setNavigationFrequency(1)) { Serial.println("GPS OK, Nav Freq 1 Hz"); freqSet = true; } 
    ...
    Serial.println("Mencari jaringan seluler (bisa beberapa menit)...");
    display.clearDisplay(); display.setCursor(0,0); display.print("NET SEARCH"); display.display();
    if (modem.waitForNetwork(300000L, true)) { 
        Serial.println("Jaringan seluler ditemukan.");
        display.setCursor(0,8); display.print("NET OK"); display.display();
    } else {
        Serial.println("Gagal menemukan jaringan seluler setelah waktu tunggu.");
        display.setCursor(0,8); display.print("NET FAIL!"); display.display();
    }
    delay(1000); 
...
void loop() {
    ...
    float gps_direct_speed_kmh = spd_mps * 3.6; // Variabel ini masih ada, tapi tidak dikirim ke JSON sederhana
    float kalman_filtered_speed_kmh = kf_estimated_speed_mps * 3.6; // Variabel ini masih ada
    // --- Akhir Baca Sensor & Proses Data ---


    // --- Logika Koneksi GPRS dan MQTT ---
    if (!modem.isNetworkConnected()) { 
        Serial.println("Koneksi jaringan seluler terputus. Cek SIM/Antena.");
        if (mqtt.connected()) mqtt.disconnect(); 
        delay(5000); // Tunggu sebelum mencoba operasi jaringan lagi
        // Tidak memanggil waitForNetwork() di sini untuk menghindari blocking loop
    } else { 
    ...
    if (sendingDataActive && mqtt.connected() && (millis() - lastLogTimeMQTT >= LOGGING_INTERVAL_MS)) {
        if (fix_type >= 2) { 
            String jsonPayload = createPeriodicJson(); // Panggil versi sederhana tanpa parameter
            
            if (jsonPayload.length() > 0) {
                Serial.print("Mempublikasikan pesan MQTT (sederhana) ke topik "); Serial.println(mqtt_topic_publish);
    ...
    if (millis() - lastSerialPrintTime > 2000) { 
        ...
        // Kecepatan masih bisa ditampilkan di Serial meskipun tidak dikirim di JSON sederhana
